...
const PdvTabs: React.FC<TPdvTabs> & TPdvTab = (props) => {
  ...
  const firstTab = Children.toArray(
    props.children
  )[0] as React.ReactElement<TTab>;

  const [selectedTab, setSelectedTab] = useState<string | number>(
    props.defaultSelectedTab ?? firstTab.props.tabKey
  );
  ...
  return (
    <>
      <div
        className={`overflow-hidden rounded-xl bg-gray-25  ${
          size === "full" ? "w-full p-1" : "inline whitespace-nowrap"
        }`}
      >
        <div
          className={`${props.className ?? ""} slider items-center rounded-xl `}
        >
          <div
            className={`flex gap-2 ${
              size === "full" ? "w-full" : ""
            } items-center rounded-xl bg-gray-25 p-2`}
          >
            {Children.map(props.children, (child: React.ReactElement<TTab>) => {
              const { children, ...restTabProps } = child.props as TTab;

              if (selectedTab === restTabProps.tabKey)
                selectedTabContent = children;
...
const Tab: React.FC<TTab> = (props) => {
  ...
  const selectedTheme = selectedVariant.includes("default")
    ? "gray-500"
    : props.color;
      ...
      >
        <PdvButton
          ...
          className={`w-full py-2.5 ${
            props.size?.includes("small") ? "px-4" : "px-10"
          } ${props.tabSize === "full" ? "grow" : ""} ${
