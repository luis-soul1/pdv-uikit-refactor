...
const useExpandableRows = <T,>({
 ...
 const isRecordExpanded = (record: T, index: number) =>
  !!expandedRows.find((row) => row.key === `${record[key]}-${index}`)?.expanded;
 ...
 const createKeys = (data: T[]) =>
  setExpandedRows(data.map((item, index) => ({ key: `${item[key]}-${index}`, expanded: false })));
